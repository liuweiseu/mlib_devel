...
class ads5296x4(YellowBlock):
    ...
    def factory(blk, plat, hdl_root=None):
        if plat.mmbus_architecture[0] == 'AXI4-Lite':
            return ads5296x4_axi4lite(blk, plat, hdl_root)
        else:
            return ads5296x4_wb(blk, plat, hdl_root)
    """
    Future common methods here.
    """

# This is for wb arch
class ads5296x4_wb(ads5296x4):
        ...
        for b in range(self.board_count):
            ...
            if b == 0:
                ...
                if self.board_count == 2:
                    ...
                    inst.add_port('fclk_in', '{adc%d_fclk0, adc%d_fclk0}' % (self.clockport, self.clockport))
...
# This is for axi4lite arch
class ads5296x4_axi4lite(ads5296x4):
    # Number of ADC chips per board
    num_units_per_board = 4
    adc_resolution = 10
    lanes_per_unit = 8
    ...
    def initialize(self):
        ...
        self.line_clock_freq_mhz = self.adc_resolution * self.sample_rate / self.lanes_per_channel / 2.0
        self.logger.info("ADC {0} has line clock {1} MHz".format(self.name, self.line_clock_freq_mhz))
        self.add_source('ads5296x4_interface_v2/ads5296x4_interface_demux2_axi4lite.v')
        self.add_source('ads5296x4_interface_v2/ads5296_unit.v')
        self.add_source('ads5296x4_interface_v2/wb_ads5296_attach.v')
        self.add_source('ads5296x4_interface_v2/data_fifo.xci')
        self.add_source('spi_master/spi_master.v')
        ...
        self.memory_map = [
            ...
            Register('fclk0_ctr',           mode='r',   offset=0x14),
            Register('fclk1_ctr',           mode='r',   offset=0x18),
            Register('fclk2_ctr',           mode='r',   offset=0x1c),
            Register('fclk3_ctr',           mode='r',   offset=0x20),
    ...
    def gen_children(self):
        ...
        #                               self.platform)
        #return [swreg]
        # TODO: The sw_reg_sync only has wb interface, we need a axi4lite interface sw_reg_sync
        rst_reg = YellowBlock.make_block({'tag':'xps:sw_reg_sync',
                                        'fullpath':'%s/ads5296_hardware_rst%d'%(self.name,self.port),
                                        'io_dir':'From Processor',
                                        'name':'ads5296_hardware_rst%d'%self.port},
    ...
    def modify_top(self,top):
        # Connect up the reset register
        # the module interfaces are changed in axi4lite arch
