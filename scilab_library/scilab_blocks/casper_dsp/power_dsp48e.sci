// power_dsp48e: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = power_dsp48e(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = power_dsp48e_update_ports(x, bconfig);
    case 'define' then
        tag = 'power_dsp48e';
        model = scicos_model();
        ...
        exprs = [];
        gr_i = [];
        // 'z' is complex (real+imag packed, width 2*bitwidth); power
        // output grows by 1 bit from squaring+adding (2*bitwidth+1).
        model.in = [1];
        model.in2 = [36];
        model.out = [1];
        model.out2 = [37];
        ...
        x = init_exprs(x);
        x.graphics.style = power_dsp48e_build_style(x.graphics.exprs(1));
        debug_info('power_dsp48e block loaded...')
    end
endfunction

function [x] = power_dsp48e_update_ports(obj, bconfigfn)
    ...
    graphics.style = power_dsp48e_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = power_dsp48e_build_style(name)
