...
function [x, y, typ] = triggered_counter(job, arg1, arg2)
    ...
    case 'define' then
        ...
        x = init_exprs(x);
        x.graphics.style = triggered_counter_build_style(x.graphics.exprs(1));
...
function [x] = triggered_counter_update_ports(obj, bconfigfn)
    ...
    graphics.style = triggered_counter_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = triggered_counter_build_style(name)
