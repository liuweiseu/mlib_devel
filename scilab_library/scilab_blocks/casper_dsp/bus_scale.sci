// bus_scale: Xcos block definition, PyQt6-mask pattern. Hand-written
...
// if unused.
function [x, y, typ] = bus_scale(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = bus_scale_update_ports(x, bconfig);
    case 'define' then
        tag = 'bus_scale';
        model = scicos_model();
        ...
        x = init_exprs(x);
        x.graphics.style = bus_scale_build_style(x.graphics.exprs(1));
        debug_info('bus_scale block loaded...')
    end
endfunction

function [x] = bus_scale_update_ports(obj, bconfigfn)
    x = obj;
    ...
    graphics.style = bus_scale_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = bus_scale_build_style(name)
