// bit_reverse: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = bit_reverse(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = bit_reverse_update_ports(x, bconfig);
    case 'define' then
        tag = 'bit_reverse';
        model = scicos_model();
        ...
        x = init_exprs(x);
        x.graphics.style = bit_reverse_build_style(x.graphics.exprs(1));
        debug_info('bit_reverse block loaded...')
    end
endfunction

function [style] = bit_reverse_build_style(name)
...
function [x] = bit_reverse_update_ports(obj, bconfigfn)
    ...
    model = x.model;
    model.in = [1];
    model.in2 = [strtod(p('n_bits'))];
    model.out = [1];
    model.out2 = [strtod(p('n_bits'))];
    graphics.in_label = ['din'];
    graphics.out_label = ['dout'];
    graphics.style = bit_reverse_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
