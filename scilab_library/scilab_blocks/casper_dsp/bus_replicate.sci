...
function [x, y, typ] = bus_replicate(job, arg1, arg2)
    ...
    case 'define' then
        ...
        x = init_exprs(x);
        x.graphics.style = bus_replicate_build_style(x.graphics.exprs(1));
...
function [x] = bus_replicate_update_ports(obj, bconfigfn)
    ...
    graphics.style = bus_replicate_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = bus_replicate_build_style(name)
