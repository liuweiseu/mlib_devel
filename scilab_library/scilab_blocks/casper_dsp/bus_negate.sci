// bus_negate: Xcos block definition, PyQt6-mask pattern. Hand-written
...
// if unused.
function [x, y, typ] = bus_negate(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = bus_negate_update_ports(x, bconfig);
    case 'define' then
        tag = 'bus_negate';
        model = scicos_model();
        ...
        x = init_exprs(x);
        x.graphics.style = bus_negate_build_style(x.graphics.exprs(1));
        debug_info('bus_negate block loaded...')
    end
endfunction

function [x] = bus_negate_update_ports(obj, bconfigfn)
    x = obj;
    ...
    model.out = [1, 2];
    model.out2 = [bus_vec_width(p('n_bits_in')), 1];
    graphics.in_label = ['din', 'misci'];
    graphics.out_label = ['dout', 'misco'];
    graphics.style = bus_negate_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = bus_negate_build_style(name)
