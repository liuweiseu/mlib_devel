// negedge_delay: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = negedge_delay(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = negedge_delay_update_ports(x, bconfig);
    case 'define' then
        tag = 'negedge_delay';
        model = scicos_model();
        ...
        x = init_exprs(x);
        x.graphics.style = negedge_delay_build_style(x.graphics.exprs(1));
        debug_info('negedge_delay block loaded...')
    end
endfunction

function [x] = negedge_delay_update_ports(obj, bconfigfn)
    ...
    graphics.style = negedge_delay_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = negedge_delay_build_style(name)
