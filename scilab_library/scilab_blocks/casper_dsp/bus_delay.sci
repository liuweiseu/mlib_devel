// bus_delay: Xcos block definition, PyQt6-mask pattern. Hand-written
...
// if unused.
function [x, y, typ] = bus_delay(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = bus_delay_update_ports(x, bconfig);
    case 'define' then
        tag = 'bus_delay';
        model = scicos_model();
        ...
        x.graphics.in_label = ['din', 'en', 'misci'];
        x.graphics.out_label = ['dout', 'misco'];
        x = init_exprs(x);
        x.graphics.style = bus_delay_build_style(x.graphics.exprs(1));
        debug_info('bus_delay block loaded...')
    end
endfunction

function [x] = bus_delay_update_ports(obj, bconfigfn)
    x = obj;
    ...
    model = x.model;
    model.in = [1, 2, 3];
    model.in2 = [bus_vec_width(p('n_bits')), 1, 1];
    model.out = [1, 2];
    model.out2 = [bus_vec_width(p('n_bits')), 1];
    graphics.in_label = ['din', 'en', 'misci'];
    graphics.out_label = ['dout', 'misco'];
    graphics.style = bus_delay_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = bus_delay_build_style(name)
