// info_block: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = info_block(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = info_block_update_ports(x, bconfig);
    case 'define' then
        tag = 'info_block';
        model = scicos_model();
        ...
        model.label = tag;
        x=standard_define([20.4 4.8],model,exprs,gr_i)
        x.graphics.in_label = [];
        x.graphics.out_label = [];
        x = init_exprs(x);
        x.graphics.style = info_block_build_style(x.graphics.exprs(1));
        debug_info('info_block block loaded...')
    end
endfunction

function [x] = info_block_update_ports(obj, bconfigfn)
    ...
    graphics.in_label = [];
    graphics.out_label = [];
    graphics.style = info_block_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = info_block_build_style(name)
