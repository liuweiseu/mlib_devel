// edge_detect: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = edge_detect(job, arg1, arg2)
    ...
    case 'set' then
        ...
        x = edge_detect_update_ports(x, bconfig);
    case 'define' then
        tag = 'edge_detect';
        model = scicos_model();
        ...
        model.label = tag;
        // icon size [4 4] scaled directly from edge_detect's own
        // casper_library icon (45x20 px), floored at MIN_ICON_UNITS --
        ...
        x = init_exprs(x);
        x.graphics.style = edge_detect_build_style(x.graphics.exprs(1));
        debug_info('edge_detect block loaded...')
    end
endfunction

function [style] = edge_detect_build_style(name)
...
function [x] = edge_detect_update_ports(obj, bconfigfn)
    ...
    graphics.style = edge_detect_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
