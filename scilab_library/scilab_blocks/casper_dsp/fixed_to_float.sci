...
function [x, y, typ] = fixed_to_float(job, arg1, arg2)
    ...
    case 'define' then
        ...
        x = init_exprs(x);
        x.graphics.style = fixed_to_float_build_style(x.graphics.exprs(1));
        debug_info('fixed_to_float block loaded...')
    end
endfunction

function [style] = fixed_to_float_build_style(name)
...
function [x] = fixed_to_float_update_ports(obj, bconfigfn)
    ...
    graphics.style = fixed_to_float_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
