// sync_gen: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = sync_gen(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = sync_gen_update_ports(x, bconfig);
    case 'define' then
        tag = 'sync_gen';
        model = scicos_model();
        ...
        x.graphics.in_label = [];
        x.graphics.out_label = ['sync'];
        x = init_exprs(x);
        x.graphics.style = sync_gen_build_style(x.graphics.exprs(1));
        debug_info('sync_gen block loaded...')
    end
endfunction

function [x] = sync_gen_update_ports(obj, bconfigfn)
    ...
    model.out2 = [1];
    graphics.in_label = [];
    graphics.out_label = ['sync'];
    graphics.style = sync_gen_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = sync_gen_build_style(name)
