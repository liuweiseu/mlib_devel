// test: Xcos block definition, PyQt6-mask pattern.
...
function [x, y, typ] = test(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
    ...
    case 'define' then
        ...
        x = init_exprs(x);
        x.graphics.style = test_build_style(x.graphics.exprs(1));
        debug_info('test block loaded...')
    end
endfunction

function [x] = test_update_ports(obj, bconfigfn)
    ...
    graphics.in_label = [];
    graphics.out_label = [];
    graphics.style = test_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = test_build_style(name)
