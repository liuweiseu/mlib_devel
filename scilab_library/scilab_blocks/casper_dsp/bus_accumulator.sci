// bus_accumulator: Xcos block definition, PyQt6-mask pattern. Hand-written
...
// if unused.
function [x, y, typ] = bus_accumulator(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = bus_accumulator_update_ports(x, bconfig);
    case 'define' then
        tag = 'bus_accumulator';
        model = scicos_model();
        ...
        x = init_exprs(x);
        x.graphics.style = bus_accumulator_build_style(x.graphics.exprs(1));
        debug_info('bus_accumulator block loaded...')
    end
endfunction

function [x] = bus_accumulator_update_ports(obj, bconfigfn)
    x = obj;
    ...
    model = x.model;
    model.in = [1, 2, 3, 4];
    model.in2 = [bus_vec_width(p('n_bits_in')), 1, 1, 1];
    model.out = [1, 2];
    model.out2 = [bus_vec_width(p('n_bits_out')), 1];
    graphics.in_label = ['din', 'rst', 'en', 'misci'];
    graphics.out_label = ['dout', 'misco'];
    graphics.style = bus_accumulator_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = bus_accumulator_build_style(name)
