// bus_relational: Xcos block definition, PyQt6-mask pattern. Hand-written
...
// if unused.
function [x, y, typ] = bus_relational(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = bus_relational_update_ports(x, bconfig);
    case 'define' then
        tag = 'bus_relational';
        model = scicos_model();
        ...
        x = init_exprs(x);
        x.graphics.style = bus_relational_build_style(x.graphics.exprs(1));
        debug_info('bus_relational block loaded...')
    end
endfunction

function [x] = bus_relational_update_ports(obj, bconfigfn)
    // dout is a single boolean compare result -- best-effort assumption, casper_library's mask has no n_bits_out for this block.
    x = obj;
    ...
    graphics.out_label = ['dout', 'misco'];
    graphics.style = bus_relational_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
...
function [style] = bus_relational_build_style(name)
