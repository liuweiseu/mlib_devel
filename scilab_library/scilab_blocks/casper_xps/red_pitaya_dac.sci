...
function [x, y, typ] = red_pitaya_dac(job, arg1, arg2)
    ...
    case 'define' then
        ...
        x = init_exprs(x);
        x.graphics.style = red_pitaya_dac_build_style(x.graphics.exprs(1));
...
function [style] = red_pitaya_dac_build_style(name)
...
function [x] = red_pitaya_dac_update_ports(obj, bconfigfn)
    ...
    graphics.out_label = [];
    graphics.style = red_pitaya_dac_build_style(p('name'));
    x.graphics = graphics;
    x.model = model;
endfunction
