// onehundred_gbe: Xcos block definition, PyQt6-mask pattern. Hand-written
// (not gen_sci_block.py) -- port list confirmed against the real
// xps_library/xps_models/IO/onehundred_gbe.slx standalone model's own top-level
// Inport/Outport blocks (names + the block's own "Ports" attribute, which
// matches this count exactly), NOT a best-effort guess.
function [x, y, typ] = onehundred_gbe(job, arg1, arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x = arg1;
        ...
        x = onehundred_gbe_update_ports(x, bconfig);
    case 'define' then
        tag = 'onehundred_gbe';
        model = scicos_model();
        ...
        model.in2 = [1, 64, 1, 32, 16, 1, 1, 1, 1, 8];
