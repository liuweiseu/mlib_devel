...
import os
from skarab_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in skarab_ui.py.
# The keys must match scilab_blocks/casper_xps/skarab.json, since the
...
class SkarabOperations(object):
    def __init__(self, winobj, template_config='skarab.json', target_config='skarab.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('skarab.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('skarab.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('SKARAB Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SKARAB_CONFIG:
                setfunc = SKARAB_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid SKARAB parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:skarab'
        for k, v in SKARAB_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for SKARAB Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='skarab.json', help='target config file.')
    ...
    win = MainWindow()
    op = SkarabOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
