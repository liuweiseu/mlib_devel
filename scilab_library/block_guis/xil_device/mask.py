...
import os
from xil_device_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in xil_device_ui.py.
# The keys must match scilab_blocks/casper_xps/xil_device.json, since the
...
class XilDeviceOperations(object):
    def __init__(self, winobj, template_config='xil_device.json', target_config='xil_device.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('xil_device.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('xil_device.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Xilinx Device Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_baseaddr(self):
        val = self.ui.baseaddr.text()
        self.logger.debug(f'baseaddr is {val}')
        return val

    def set_baseaddr(self, val):
        self.logger.debug(f'Set baseaddr to {val}')
        self.ui.baseaddr.setText(str(val))

    def get_memsize(self):
        val = self.ui.memsize.text()
        self.logger.debug(f'memsize is {val}')
        return val

    def set_memsize(self, val):
        self.logger.debug(f'Set memsize to {val}')
        self.ui.memsize.setText(str(val))

    def get_devtype(self):
        text = self.ui.devtype.currentText()
        self.logger.debug(f'devtype is {text}')
        return text

    def set_devtype(self, text):
        self.logger.debug(f'Set devtype to {text}')
        self.ui.devtype.setCurrentText(str(text))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_baseaddr()).strip() == '':
            errors.append('BaseAddress must not be empty.')
        if str(self.get_memsize()).strip() == '':
            errors.append('MemorySize must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in XIL_DEVICE_CONFIG:
                setfunc = XIL_DEVICE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Xilinx Device parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:xil_device'
        for k, v in XIL_DEVICE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Xilinx Device Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='xil_device.json', help='target config file.')
    ...
    win = MainWindow()
    op = XilDeviceOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
