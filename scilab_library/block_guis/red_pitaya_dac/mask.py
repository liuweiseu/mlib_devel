...
import os
from red_pitaya_dac_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in red_pitaya_dac_ui.py.
# The keys must match scilab_blocks/casper_xps/red_pitaya_dac.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
RED_PITAYA_DAC_CONFIG = {
    'bits':
    {
        'get': 'get_bits',
        'set': 'set_bits'
...
class RedPitayaDacOperations(object):
    def __init__(self, winobj, template_config='red_pitaya_dac.json', target_config='red_pitaya_dac.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('red_pitaya_dac.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('red_pitaya_dac.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Red Pitaya DAC Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in RED_PITAYA_DAC_CONFIG:
                setfunc = RED_PITAYA_DAC_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Red Pitaya DAC parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:red_pitaya_dac'
        for k, v in RED_PITAYA_DAC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Red Pitaya DAC Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='red_pitaya_dac.json', help='target config file.')
    ...
    win = MainWindow()
    op = RedPitayaDacOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
