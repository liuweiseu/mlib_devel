...
import os
from adc_16g_asnt_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc_16g_asnt_ui.py.
# The keys must match scilab_blocks/casper_xps/adc_16g_asnt.json, since the
...
class Adc16gAsntOperations(object):
    def __init__(self, winobj, template_config='adc_16g_asnt.json', target_config='adc_16g_asnt.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc_16g_asnt.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc_16g_asnt.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc_16g_asnt Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_channel_sel(self):
        text = self.ui.channel_sel.currentText()
        self.logger.debug(f'channel_sel is {text}')
        return text

    def set_channel_sel(self, text):
        self.logger.debug(f'Set channel_sel to {text}')
        self.ui.channel_sel.setCurrentText(str(text))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC_16G_ASNT_CONFIG:
                setfunc = ADC_16G_ASNT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc_16g_asnt parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc_16g_asnt'
        for k, v in ADC_16G_ASNT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc_16g_asnt Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc_16g_asnt.json', help='target config file.')
    ...
    win = MainWindow()
    op = Adc16gAsntOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
