...
import os
from adc_sim_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc_sim_ui.py.
# The keys must match scilab_blocks/casper_xps/adc_sim.json, since the
...
class AdcSimOperations(object):
    def __init__(self, winobj, template_config='adc_sim.json', target_config='adc_sim.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc_sim.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc_sim.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc_sim Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_nStreams()).strip() == '':
            errors.append('Number of Output Streams must not be empty.')
        if str(self.get_bit_width()).strip() == '':
            errors.append('Sample Bit Width must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC_SIM_CONFIG:
                setfunc = ADC_SIM_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc_sim parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc_sim'
        for k, v in ADC_SIM_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc_sim Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc_sim.json', help='target config file.')
    ...
    win = MainWindow()
    op = AdcSimOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
