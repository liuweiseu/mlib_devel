...
import os
from triggered_counter_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in triggered_counter_ui.py.
# The keys must match scilab_blocks/casper_dsp/triggered_counter.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
TRIGGERED_COUNTER_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class TriggeredCounterOperations(object):
    def __init__(self, winobj, template_config='triggered_counter.json', target_config='triggered_counter.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('triggered_counter.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('triggered_counter.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Triggered Counter Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_len(self):
        val = self.ui.len.value()
        self.logger.debug(f'len is {val}')
        return str(val)

    def set_len(self, val):
        self.logger.debug(f'Set len to {val}')
        self.ui.len.setValue(int(val))

    def get_count_width(self):
        val = self.ui.count_width.value()
        self.logger.debug(f'count_width is {val}')
        return str(val)

    def set_count_width(self, val):
        self.logger.debug(f'Set count_width to {val}')
        self.ui.count_width.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in TRIGGERED_COUNTER_CONFIG:
                setfunc = TRIGGERED_COUNTER_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Triggered Counter parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:triggered_counter'
        for k, v in TRIGGERED_COUNTER_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Triggered Counter Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='triggered_counter.json', help='target config file.')
    ...
    win = MainWindow()
    op = TriggeredCounterOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
