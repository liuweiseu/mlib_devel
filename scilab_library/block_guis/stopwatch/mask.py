...
import os
from stopwatch_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in stopwatch_ui.py.
# The keys must match scilab_blocks/casper_dsp/stopwatch.json, since the
...
class StopwatchOperations(object):
    def __init__(self, winobj, template_config='stopwatch.json', target_config='stopwatch.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('stopwatch.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('stopwatch.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Stopwatch Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in STOPWATCH_CONFIG:
                setfunc = STOPWATCH_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Stopwatch parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:stopwatch'
        for k, v in STOPWATCH_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Stopwatch Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='stopwatch.json', help='target config file.')
    ...
    win = MainWindow()
    op = StopwatchOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
