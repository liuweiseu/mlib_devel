...
import os
from i2c_interface_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in i2c_interface_ui.py.
...
I2C_INTERFACE_CONFIG = {
    ...
    'sda_gpio_index':
    {
        'get': 'get_sda_gpio_index',
        'set': 'set_sda_gpio_index'
    },
    'scl_gpio':
    {
        'get': 'get_scl_gpio',
    ...
    'scl_gpio_index':
    {
        'get': 'get_scl_gpio_index',
        'set': 'set_scl_gpio_index'
    },
}
...
class I2cInterfaceOperations(object):
    def __init__(self, winobj, template_config='i2c_interface.json', target_config='i2c_interface.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('i2c_interface.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('i2c_interface.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('I2C Interface Mask started.')
        self.logger.info('*************************************************')
    ...
    def set_sda_gpio(self, val):
        self.logger.debug(f'Set sda_gpio to {val}')
        self.ui.sda_gpio.setText(str(val))

    def get_sda_gpio_index(self):
        val = self.ui.sda_gpio_index.value()
        self.logger.debug(f'sda_gpio_index is {val}')
        return str(val)

    def set_sda_gpio_index(self, val):
        self.logger.debug(f'Set sda_gpio_index to {val}')
        self.ui.sda_gpio_index.setValue(int(val))

    def get_scl_gpio(self):
        val = self.ui.scl_gpio.text()
        self.logger.debug(f'scl_gpio is {val}')
        return val

    def set_scl_gpio(self, val):
        self.logger.debug(f'Set scl_gpio to {val}')
        self.ui.scl_gpio.setText(str(val))

    def get_scl_gpio_index(self):
        val = self.ui.scl_gpio_index.value()
        self.logger.debug(f'scl_gpio_index is {val}')
        return str(val)

    def set_scl_gpio_index(self, val):
        self.logger.debug(f'Set scl_gpio_index to {val}')
        self.ui.scl_gpio_index.setValue(int(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_sda_gpio()).strip() == '':
            errors.append('SDA GPIO block name must not be empty.')
        if str(self.get_scl_gpio()).strip() == '':
            errors.append('SCL GPIO block name must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in I2C_INTERFACE_CONFIG:
                setfunc = I2C_INTERFACE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid I2C Interface parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:i2c_interface'
        for k, v in I2C_INTERFACE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for I2C Interface Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='i2c_interface.json', help='target config file.')
    ...
    win = MainWindow()
    op = I2cInterfaceOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
