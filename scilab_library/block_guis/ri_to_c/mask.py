...
import os
from ri_to_c_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in ri_to_c_ui.py.
# The keys must match scilab_blocks/casper_dsp/ri_to_c.json, since the
...
class RiToCOperations(object):
    def __init__(self, winobj, template_config='ri_to_c.json', target_config='ri_to_c.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('ri_to_c.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('ri_to_c.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Real-Imag to Complex Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in RI_TO_C_CONFIG:
                setfunc = RI_TO_C_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Real-Imag to Complex parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:ri_to_c'
        for k, v in RI_TO_C_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Real-Imag to Complex Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='ri_to_c.json', help='target config file.')
    ...
    win = MainWindow()
    op = RiToCOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
