...
import os
from vacc_tvg_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in vacc_tvg_ui.py.
# The keys must match scilab_blocks/casper_dsp/vacc_tvg.json, since the
...
class VaccTvgOperations(object):
    def __init__(self, winobj, template_config='vacc_tvg.json', target_config='vacc_tvg.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('vacc_tvg.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('vacc_tvg.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Vector Accumulator Test Vector Generator Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_len()).strip() == '':
            errors.append('Number of Vectors must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in VACC_TVG_CONFIG:
                setfunc = VACC_TVG_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Vector Accumulator Test Vector Generator parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:vacc_tvg'
        for k, v in VACC_TVG_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Vector Accumulator Test Vector Generator Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='vacc_tvg.json', help='target config file.')
    ...
    win = MainWindow()
    op = VaccTvgOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
