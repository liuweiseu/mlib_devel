...
import os
from vcu118_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in vcu118_ui.py.
# The keys must match scilab_blocks/casper_xps/vcu118.json, since the
...
class Vcu118Operations(object):
    def __init__(self, winobj, template_config='vcu118.json', target_config='vcu118.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('vcu118.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('vcu118.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('VCU118 Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_axi_master(self):
        text = self.ui.axi_master.currentText()
        self.logger.debug(f'axi_master is {text}')
        return text

    def set_axi_master(self, text):
        self.logger.debug(f'Set axi_master to {text}')
        self.ui.axi_master.setCurrentText(str(text))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in VCU118_CONFIG:
                setfunc = VCU118_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid VCU118 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:vcu118'
        for k, v in VCU118_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for VCU118 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='vcu118.json', help='target config file.')
    ...
    win = MainWindow()
    op = Vcu118Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
