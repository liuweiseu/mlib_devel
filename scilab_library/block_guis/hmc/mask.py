...
import os
from hmc_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in hmc_ui.py.
# The keys must match scilab_blocks/casper_xps/hmc.json, since the
...
class HmcOperations(object):
    def __init__(self, winobj, template_config='hmc.json', target_config='hmc.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('hmc.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('hmc.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('HMC Mask started.')
        self.logger.info('*************************************************')
    ...
    # *******************************************************************
    def get_mez(self):
        text = self.ui.mez.currentText()
        self.logger.debug(f'mez is {text}')
        return text

    def set_mez(self, text):
        self.logger.debug(f'Set mez to {text}')
        self.ui.mez.setCurrentText(str(text))

    def get_sim_depth(self):
        val = self.ui.sim_depth.value()
        self.logger.debug(f'sim_depth is {val}')
        return str(val)

    def set_sim_depth(self, val):
        self.logger.debug(f'Set sim_depth to {val}')
        self.ui.sim_depth.setValue(int(val))

    def get_sim_latency(self):
        val = self.ui.sim_latency.value()
        self.logger.debug(f'sim_latency is {val}')
        return str(val)

    def set_sim_latency(self, val):
        self.logger.debug(f'Set sim_latency to {val}')
        self.ui.sim_latency.setValue(int(val))

    def get_sim_rand(self):
        status = self.ui.sim_rand.isChecked()
        self.logger.debug(f'sim_rand status is {status}')
        return self._convert_status(status)

    def set_sim_rand(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set sim_rand status to {status}')
        self.ui.sim_rand.setChecked(status)
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid HMC parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:hmc'
        for k, v in HMC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for HMC Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='hmc.json', help='target config file.')
    ...
    win = MainWindow()
    op = HmcOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
