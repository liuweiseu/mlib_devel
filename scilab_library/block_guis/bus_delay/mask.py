...
import os
from bus_delay_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_delay_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_delay.json, since the
...
BUS_DELAY_CONFIG = {
    ...
    'reg_retiming':
    {
        'get': 'get_reg_retiming',
        'set': 'set_reg_retiming'
    },
    'misc':
    {
        'get': 'get_misc',
...
class BusDelayOperations(object):
    def __init__(self, winobj, template_config='bus_delay.json', target_config='bus_delay.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_delay.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_delay.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Delay Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_reg_retiming(self):
        status = self.ui.reg_retiming.isChecked()
        self.logger.debug(f'reg_retiming status is {status}')
        return self._convert_status(status)

    def set_reg_retiming(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set reg_retiming status to {status}')
        self.ui.reg_retiming.setChecked(status)
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_DELAY_CONFIG:
                setfunc = BUS_DELAY_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Delay parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_delay'
        for k, v in BUS_DELAY_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Delay Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_delay.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusDelayOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
