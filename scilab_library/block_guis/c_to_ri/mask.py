...
import os
from c_to_ri_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in c_to_ri_ui.py.
# The keys must match scilab_blocks/casper_dsp/c_to_ri.json, since the
...
class CToRiOperations(object):
    def __init__(self, winobj, template_config='c_to_ri.json', target_config='c_to_ri.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('c_to_ri.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('c_to_ri.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Complex to Real-Imag Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in C_TO_RI_CONFIG:
                setfunc = C_TO_RI_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Complex to Real-Imag parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:c_to_ri'
        for k, v in C_TO_RI_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Complex to Real-Imag Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='c_to_ri.json', help='target config file.')
    ...
    win = MainWindow()
    op = CToRiOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
