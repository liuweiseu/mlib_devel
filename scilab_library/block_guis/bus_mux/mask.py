...
import os
from bus_mux_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_mux_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_mux.json, since the
...
class BusMuxOperations(object):
    def __init__(self, winobj, template_config='bus_mux.json', target_config='bus_mux.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_mux.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_mux.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Mux Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_mux_latency(self):
        val = self.ui.mux_latency.text()
        self.logger.debug(f'mux_latency is {val}')
        return val

    def set_mux_latency(self, val):
        self.logger.debug(f'Set mux_latency to {val}')
        self.ui.mux_latency.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        n_inputs_str = str(self.get_n_inputs()).strip()
        if n_inputs_str == '':
            errors.append('number of inputs must not be empty.')
        else:
            try:
                if int(n_inputs_str) < 1:
                    errors.append('number of inputs must be at least 1 (it scales the number of Xcos ports).')
            except ValueError:
                errors.append('number of inputs must be an integer.')
        if str(self.get_n_bits()).strip() == '':
            errors.append('input bit widths must not be empty.')
        if str(self.get_mux_latency()).strip() == '':
            errors.append('mux latency must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_MUX_CONFIG:
                setfunc = BUS_MUX_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Mux parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_mux'
        for k, v in BUS_MUX_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Mux Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_mux.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusMuxOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
