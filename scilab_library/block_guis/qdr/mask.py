...
import os
from qdr_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in qdr_ui.py.
# The keys must match scilab_blocks/casper_xps/qdr.json, since the
...
class QdrOperations(object):
    def __init__(self, winobj, template_config='qdr.json', target_config='qdr.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('qdr.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('qdr.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('QDR Mask started.')
        self.logger.info('*************************************************')
    ...
    # Low-level APIs
    # *******************************************************************
    def get_which_qdr(self):
        text = self.ui.which_qdr.currentText()
        self.logger.debug(f'which_qdr is {text}')
        return text

    def set_which_qdr(self, text):
        self.logger.debug(f'Set which_qdr to {text}')
        self.ui.which_qdr.setCurrentText(str(text))

    def get_qdr_awidth(self):
        val = self.ui.qdr_awidth.value()
        self.logger.debug(f'qdr_awidth is {val}')
        return str(val)

    def set_qdr_awidth(self, val):
        self.logger.debug(f'Set qdr_awidth to {val}')
        self.ui.qdr_awidth.setValue(int(val))
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid QDR parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:qdr'
        for k, v in QDR_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for QDR Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='qdr.json', help='target config file.')
    ...
    win = MainWindow()
    op = QdrOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
