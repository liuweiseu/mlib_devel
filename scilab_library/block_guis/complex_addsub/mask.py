...
import os
from complex_addsub_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in complex_addsub_ui.py.
# The keys must match scilab_blocks/casper_dsp/complex_addsub.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
COMPLEX_ADDSUB_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class ComplexAddsubOperations(object):
    def __init__(self, winobj, template_config='complex_addsub.json', target_config='complex_addsub.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('complex_addsub.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('complex_addsub.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Complex Add/Sub Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in COMPLEX_ADDSUB_CONFIG:
                setfunc = COMPLEX_ADDSUB_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Complex Add/Sub parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:complex_addsub'
        for k, v in COMPLEX_ADDSUB_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Complex Add/Sub Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='complex_addsub.json', help='target config file.')
    ...
    win = MainWindow()
    op = ComplexAddsubOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
