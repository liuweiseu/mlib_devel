...
import os
from adc_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc_ui.py.
# The keys must match scilab_blocks/casper_xps/adc.json, since the
...
class AdcOperations(object):
    def __init__(self, winobj, template_config='adc.json', target_config='adc.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc Mask started.')
        self.logger.info('*************************************************')
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc'
        for k, v in ADC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc.json', help='target config file.')
    ...
    win = MainWindow()
    op = AdcOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
