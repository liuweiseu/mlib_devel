...
import os
from shifter_unit_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in shifter_unit_ui.py.
# The keys must match scilab_blocks/casper_dsp/shifter_unit.json, since the
...
class ShifterUnitOperations(object):
    def __init__(self, winobj, template_config='shifter_unit.json', target_config='shifter_unit.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('shifter_unit.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('shifter_unit.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Shifter Unit Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_pin_offset(self):
        val = self.ui.pin_offset.value()
        self.logger.debug(f'pin_offset is {val}')
        return str(val)

    def set_pin_offset(self, val):
        self.logger.debug(f'Set pin_offset to {val}')
        self.ui.pin_offset.setValue(int(val))
    ...
    def validate_config(self):
        ...
        data_width/pin_offset are spinboxes already range-constrained (>=1 /
        >=0); shifter_unit has no other mask-level validation of its own in
        casper_library (all cross-parameter checks live in the parent
        parallel_to_serial_converter's mask instead).
        """
        errors = []
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SHIFTER_UNIT_CONFIG:
                setfunc = SHIFTER_UNIT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Shifter Unit parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:shifter_unit'
        for k, v in SHIFTER_UNIT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Shifter Unit Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='shifter_unit.json', help='target config file.')
    ...
    win = MainWindow()
    op = ShifterUnitOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
