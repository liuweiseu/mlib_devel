...
import os
from onehundred_gbe_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in onehundred_gbe_ui.py.
# The keys must match scilab_blocks/casper_xps/onehundred_gbe.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
ONEHUNDRED_GBE_CONFIG = {
    'flavour':
    {
        'get': 'get_flavour',
        'set': 'set_flavour'
...
class OnehundredGbeOperations(object):
    def __init__(self, winobj, template_config='onehundred_gbe.json', target_config='onehundred_gbe.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('onehundred_gbe.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('onehundred_gbe.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('One Hundred GbE Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_port()).strip() == '':
            errors.append('Port must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ONEHUNDRED_GBE_CONFIG:
                setfunc = ONEHUNDRED_GBE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid One Hundred GbE parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:onehundred_gbe'
        for k, v in ONEHUNDRED_GBE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for One Hundred GbE Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='onehundred_gbe.json', help='target config file.')
    ...
    win = MainWindow()
    op = OnehundredGbeOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
