...
import os
from info_block_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in info_block_ui.py.
# The keys must match scilab_blocks/casper_dsp/info_block.json, since the
...
class InfoBlockOperations(object):
    def __init__(self, winobj, template_config='info_block.json', target_config='info_block.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('info_block.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('info_block.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Info Block Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_info(self):
        val = self.ui.info.text()
        self.logger.debug(f'info is {val}')
        return val

    def set_info(self, val):
        self.logger.debug(f'Set info to {val}')
        self.ui.info.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_info()).strip() == '':
            errors.append('Info: must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in INFO_BLOCK_CONFIG:
                setfunc = INFO_BLOCK_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Info Block parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:info_block'
        for k, v in INFO_BLOCK_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Info Block Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='info_block.json', help='target config file.')
    ...
    win = MainWindow()
    op = InfoBlockOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
