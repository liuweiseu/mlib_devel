...
import os
from fixed_to_float_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in fixed_to_float_ui.py.
# The keys must match scilab_blocks/casper_dsp/fixed_to_float.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
FIXED_TO_FLOAT_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class FixedToFloatOperations(object):
    def __init__(self, winobj, template_config='fixed_to_float.json', target_config='fixed_to_float.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('fixed_to_float.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('fixed_to_float.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Fixed to Float Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_num_vec(self):
        val = self.ui.num_vec.value()
        self.logger.debug(f'num_vec is {val}')
        return str(val)

    def set_num_vec(self, val):
        self.logger.debug(f'Set num_vec to {val}')
        self.ui.num_vec.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in FIXED_TO_FLOAT_CONFIG:
                setfunc = FIXED_TO_FLOAT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Fixed to Float parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:fixed_to_float'
        for k, v in FIXED_TO_FLOAT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Fixed to Float Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='fixed_to_float.json', help='target config file.')
    ...
    win = MainWindow()
    op = FixedToFloatOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
