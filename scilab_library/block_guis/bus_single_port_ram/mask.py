...
import os
from bus_single_port_ram_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_single_port_ram_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_single_port_ram.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
BUS_SINGLE_PORT_RAM_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
    ...
    'addr_register':
    {
        'get': 'get_addr_register',
        'set': 'set_addr_register'
    },
    'addr_implementation':
    {
        'get': 'get_addr_implementation',
        'set': 'set_addr_implementation'
    },
    'din_register':
    {
        'get': 'get_din_register',
        'set': 'set_din_register'
    },
    'din_implementation':
    {
        'get': 'get_din_implementation',
        'set': 'set_din_implementation'
    },
    'we_register':
    {
        'get': 'get_we_register',
    ...
    'we_implementation':
    {
        'get': 'get_we_implementation',
        'set': 'set_we_implementation'
    },
    'en_register':
    {
        'get': 'get_en_register',
    ...
    'en_implementation':
    {
        'get': 'get_en_implementation',
        'set': 'set_en_implementation'
    },
}
...
class BusSinglePortRamOperations(object):
    def __init__(self, winobj, template_config='bus_single_port_ram.json', target_config='bus_single_port_ram.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_single_port_ram.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_single_port_ram.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Single Port RAM Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_async_op(self):
        status = self.ui.async_op.isChecked()
        self.logger.debug(f'async_op status is {status}')
        return self._convert_status(status)

    def set_async_op(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set async_op status to {status}')
        self.ui.async_op.setChecked(status)
    ...
    def get_addr_register(self):
        status = self.ui.addr_register.isChecked()
        self.logger.debug(f'addr_register status is {status}')
        return self._convert_status(status)

    def set_addr_register(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set addr_register status to {status}')
        self.ui.addr_register.setChecked(status)

    def get_addr_implementation(self):
        text = self.ui.addr_implementation.currentText()
        self.logger.debug(f'addr_implementation is {text}')
        return text

    def set_addr_implementation(self, text):
        self.logger.debug(f'Set addr_implementation to {text}')
        self.ui.addr_implementation.setCurrentText(str(text))

    def get_din_register(self):
        status = self.ui.din_register.isChecked()
        self.logger.debug(f'din_register status is {status}')
        return self._convert_status(status)

    def set_din_register(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set din_register status to {status}')
        self.ui.din_register.setChecked(status)

    def get_din_implementation(self):
        text = self.ui.din_implementation.currentText()
        self.logger.debug(f'din_implementation is {text}')
        return text

    def set_din_implementation(self, text):
        self.logger.debug(f'Set din_implementation to {text}')
        self.ui.din_implementation.setCurrentText(str(text))

    def get_we_register(self):
        status = self.ui.we_register.isChecked()
        self.logger.debug(f'we_register status is {status}')
        return self._convert_status(status)

    def set_we_register(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set we_register status to {status}')
        self.ui.we_register.setChecked(status)

    def get_we_implementation(self):
        text = self.ui.we_implementation.currentText()
        self.logger.debug(f'we_implementation is {text}')
        return text

    def set_we_implementation(self, text):
        self.logger.debug(f'Set we_implementation to {text}')
        self.ui.we_implementation.setCurrentText(str(text))

    def get_en_register(self):
        status = self.ui.en_register.isChecked()
        self.logger.debug(f'en_register status is {status}')
        return self._convert_status(status)

    def set_en_register(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set en_register status to {status}')
        self.ui.en_register.setChecked(status)

    def get_en_implementation(self):
        text = self.ui.en_implementation.currentText()
        self.logger.debug(f'en_implementation is {text}')
        return text

    def set_en_implementation(self, text):
        self.logger.debug(f'Set en_implementation to {text}')
        self.ui.en_implementation.setCurrentText(str(text))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_SINGLE_PORT_RAM_CONFIG:
                setfunc = BUS_SINGLE_PORT_RAM_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Single Port RAM parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_single_port_ram'
        for k, v in BUS_SINGLE_PORT_RAM_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Single Port RAM Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_single_port_ram.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusSinglePortRamOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
