...
import os
from bus_adder_tree_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_adder_tree_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_adder_tree.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
BUS_ADDER_TREE_CONFIG = {
    'n_busses':
    {
        'get': 'get_n_busses',
        'set': 'set_n_busses'
    ...
    'num_float_vec':
    {
        'get': 'get_num_float_vec',
        'set': 'set_num_float_vec'
    },
    'bin_pts':
    {
        'get': 'get_bin_pts',
    ...
    'first_stage_hdl':
    {
        'get': 'get_first_stage_hdl',
        'set': 'set_first_stage_hdl'
    },
    'adder_imp':
    {
        'get': 'get_adder_imp',
...
class BusAdderTreeOperations(object):
    def __init__(self, winobj, template_config='bus_adder_tree.json', target_config='bus_adder_tree.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_adder_tree.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_adder_tree.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Adder Tree Mask started.')
        self.logger.info('*************************************************')
    ...
    # Low-level APIs
    # *******************************************************************
    def get_n_busses(self):
        val = self.ui.n_busses.text()
        self.logger.debug(f'n_busses is {val}')
        return val

    def set_n_busses(self, val):
        self.logger.debug(f'Set n_busses to {val}')
        self.ui.n_busses.setText(str(val))
    ...
    def get_num_float_vec(self):
        val = self.ui.num_float_vec.text()
        self.logger.debug(f'num_float_vec is {val}')
        return val

    def set_num_float_vec(self, val):
        self.logger.debug(f'Set num_float_vec to {val}')
        self.ui.num_float_vec.setText(str(val))
    ...
    def get_dtypes(self):
        val = self.ui.dtypes.text()
        self.logger.debug(f'dtypes is {val}')
        return val

    def set_dtypes(self, val):
        self.logger.debug(f'Set dtypes to {val}')
        self.ui.dtypes.setText(str(val))
    ...
    def get_first_stage_hdl(self):
        status = self.ui.first_stage_hdl.isChecked()
        self.logger.debug(f'first_stage_hdl status is {status}')
        return self._convert_status(status)

    def set_first_stage_hdl(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set first_stage_hdl status to {status}')
        self.ui.first_stage_hdl.setChecked(status)

    def get_adder_imp(self):
        text = self.ui.adder_imp.currentText()
        self.logger.debug(f'adder_imp is {text}')
        return text

    def set_adder_imp(self, text):
        self.logger.debug(f'Set adder_imp to {text}')
        self.ui.adder_imp.setCurrentText(str(text))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        n_busses_str = str(self.get_n_busses()).strip()
        if n_busses_str == '':
            errors.append('number of busses must not be empty.')
        else:
            try:
                if int(n_busses_str) < 1:
                    errors.append('number of busses must be at least 1 (it scales the number of Xcos ports).')
            except ValueError:
                errors.append('number of busses must be an integer.')
        if str(self.get_n_bits()).strip() == '':
            errors.append('bus component bit widths must not be empty.')
        if str(self.get_num_float_vec()).strip() == '':
            errors.append('number floating point vectors in bus must not be empty.')
        if str(self.get_bin_pts()).strip() == '':
            errors.append('bus component binary points must not be empty.')
        if str(self.get_dtypes()).strip() == '':
            errors.append('bus component data types must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_ADDER_TREE_CONFIG:
                setfunc = BUS_ADDER_TREE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Adder Tree parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_adder_tree'
        for k, v in BUS_ADDER_TREE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Adder Tree Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_adder_tree.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusAdderTreeOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
