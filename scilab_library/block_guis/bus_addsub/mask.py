...
import os
from bus_addsub_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_addsub_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_addsub.json, since the
...
class BusAddsubOperations(object):
    def __init__(self, winobj, template_config='bus_addsub.json', target_config='bus_addsub.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_addsub.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_addsub.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Add/Sub Mask started.')
        self.logger.info('*************************************************')
    ...
    # *******************************************************************
    def get_opmode(self):
        val = self.ui.opmode.text()
        self.logger.debug(f'opmode is {val}')
        return val

    def set_opmode(self, val):
        self.logger.debug(f'Set opmode to {val}')
        self.ui.opmode.setText(str(val))
    ...
    def get_pipeline_en(self):
        status = self.ui.pipeline_en.isChecked()
        self.logger.debug(f'pipeline_en status is {status}')
        return self._convert_status(status)

    def set_pipeline_en(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set pipeline_en status to {status}')
        self.ui.pipeline_en.setChecked(status)
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_opmode()).strip() == '':
            errors.append('mode (Addition=0, Subtraction=1) must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_ADDSUB_CONFIG:
                setfunc = BUS_ADDSUB_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Add/Sub parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_addsub'
        for k, v in BUS_ADDSUB_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Add/Sub Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_addsub.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusAddsubOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
