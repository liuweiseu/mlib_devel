...
import os
from vacc_async_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in vacc_async_ui.py.
# The keys must match scilab_blocks/casper_dsp/vacc_async.json, since the
...
class VaccAsyncOperations(object):
    def __init__(self, winobj, template_config='vacc_async.json', target_config='vacc_async.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('vacc_async.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('vacc_async.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Async Vector Accumulator Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        Rules ported from casper_library/simple_bram_vacc_init.m (this
        block shares the same mask_type/init_call).
        """
        errors = []
        vec_len_str = str(self.get_vec_len()).strip()
        if vec_len_str == '':
            errors.append('Vector length (2^? samples): must not be empty.')
        else:
            try:
                vec_len = int(vec_len_str)
            except ValueError:
                errors.append('Vector length (2^? samples): must be an integer.')
            else:
                if vec_len < 6:
                    errors.append('Vector length (2^? samples): must be greater than 5.')
        n_bits = int(self.get_n_bits())
        if n_bits < 1:
            errors.append('Bit width (output): must be greater than 0.')
        bin_pt = int(self.get_bin_pt())
        if bin_pt > n_bits:
            errors.append('Binary point (output): cannot be greater than the bit width.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in VACC_ASYNC_CONFIG:
                setfunc = VACC_ASYNC_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Async Vector Accumulator parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:vacc_async'
        for k, v in VACC_ASYNC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Async Vector Accumulator Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='vacc_async.json', help='target config file.')
    ...
    win = MainWindow()
    op = VaccAsyncOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
