...
import os
from convert_of_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in convert_of_ui.py.
# The keys must match scilab_blocks/casper_dsp/convert_of.json, since the
...
CONVERT_OF_CONFIG = {
    ...
    'binary_point_i':
    {
        'get': 'get_binary_point_i',
        'set': 'set_binary_point_i'
    },
    'bit_width_o':
    {
        'get': 'get_bit_width_o',
    ...
    'binary_point_o':
    {
        'get': 'get_binary_point_o',
        'set': 'set_binary_point_o'
    },
    'quantization':
    {
...
class ConvertOfOperations(object):
    def __init__(self, winobj, template_config='convert_of.json', target_config='convert_of.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('convert_of.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('convert_of.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Convert with Overflow Mask started.')
        self.logger.info('*************************************************')
    ...
    # Low-level APIs
    # *******************************************************************
    def get_bit_width_i(self):
        val = self.ui.bit_width_i.value()
        self.logger.debug(f'bit_width_i is {val}')
        return str(val)

    def set_bit_width_i(self, val):
        self.logger.debug(f'Set bit_width_i to {val}')
        self.ui.bit_width_i.setValue(int(val))

    def get_binary_point_i(self):
        val = self.ui.binary_point_i.value()
        self.logger.debug(f'binary_point_i is {val}')
        return str(val)

    def set_binary_point_i(self, val):
        self.logger.debug(f'Set binary_point_i to {val}')
        self.ui.binary_point_i.setValue(int(val))

    def get_bit_width_o(self):
        val = self.ui.bit_width_o.value()
        self.logger.debug(f'bit_width_o is {val}')
        return str(val)

    def set_bit_width_o(self, val):
        self.logger.debug(f'Set bit_width_o to {val}')
        self.ui.bit_width_o.setValue(int(val))

    def get_binary_point_o(self):
        val = self.ui.binary_point_o.value()
        self.logger.debug(f'binary_point_o is {val}')
        return str(val)

    def set_binary_point_o(self, val):
        self.logger.debug(f'Set binary_point_o to {val}')
        self.ui.binary_point_o.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in CONVERT_OF_CONFIG:
                setfunc = CONVERT_OF_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Convert with Overflow parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:convert_of'
        for k, v in CONVERT_OF_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Convert with Overflow Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='convert_of.json', help='target config file.')
    ...
    win = MainWindow()
    op = ConvertOfOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
