...
import os
from adc1x1800_10_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc1x1800_10_ui.py.
# The keys must match scilab_blocks/casper_xps/adc1x1800_10.json, since the
...
class Adc1x180010Operations(object):
    def __init__(self, winobj, template_config='adc1x1800_10.json', target_config='adc1x1800_10.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc1x1800_10.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc1x1800_10.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc1x1800_10 Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_gray_en(self):
        status = self.ui.gray_en.isChecked()
        self.logger.debug(f'gray_en status is {status}')
        return self._convert_status(status)

    def set_gray_en(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set gray_en status to {status}')
        self.ui.gray_en.setChecked(status)
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC1X1800_10_CONFIG:
                setfunc = ADC1X1800_10_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc1x1800_10 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc1x1800_10'
        for k, v in ADC1X1800_10_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc1x1800_10 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc1x1800_10.json', help='target config file.')
    ...
    win = MainWindow()
    op = Adc1x180010Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
