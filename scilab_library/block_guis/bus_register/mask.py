...
import os
from bus_register_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_register_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_register.json, since the
...
class BusRegisterOperations(object):
    def __init__(self, winobj, template_config='bus_register.json', target_config='bus_register.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_register.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_register.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Register Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_REGISTER_CONFIG:
                setfunc = BUS_REGISTER_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Register parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_register'
        for k, v in BUS_REGISTER_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Register Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_register.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusRegisterOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
