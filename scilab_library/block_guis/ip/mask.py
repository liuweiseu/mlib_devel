...
import os
from ip_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in ip_ui.py.
...
class IpOperations(object):
    def __init__(self, winobj, template_config='ip.json', target_config='ip.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('ip.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('ip.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('IP Mask started.')
        self.logger.info('*************************************************')
    ...
    def set_lib_path(self, val):
        self.logger.debug(f'Set lib_path to {val}')
        self.ui.lib_path.setText(str(val))

    def get_ip_name(self):
        val = self.ui.ip_name.text()
        self.logger.debug(f'ip_name is {val}')
        return val

    def set_ip_name(self, val):
        self.logger.debug(f'Set ip_name to {val}')
        self.ui.ip_name.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_lib_path()).strip() == '':
            errors.append('Path to sysgen build directory (the directory containing ip/) must not be empty.')
        if str(self.get_ip_name()).strip() == '':
            errors.append('Name of IP in this library must not be empty.')
        return errors
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid IP parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:ip'
        for k, v in IP_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for IP Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='ip.json', help='target config file.')
    ...
    win = MainWindow()
    op = IpOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
