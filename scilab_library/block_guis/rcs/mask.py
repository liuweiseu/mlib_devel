...
import os
from rcs_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in rcs_ui.py.
# The keys must match scilab_blocks/casper_dsp/rcs.json, since the
...
class RcsOperations(object):
    def __init__(self, winobj, template_config='rcs.json', target_config='rcs.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('rcs.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('rcs.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Revision Control System Mask started.')
        self.logger.info('*************************************************')
    ...
    # Low-level APIs
    # *******************************************************************
    def get_app_src(self):
        text = self.ui.app_src.currentText()
        self.logger.debug(f'app_src is {text}')
        return text

    def set_app_src(self, text):
        self.logger.debug(f'Set app_src to {text}')
        self.ui.app_src.setCurrentText(str(text))

    def get_lib_src(self):
        text = self.ui.lib_src.currentText()
        self.logger.debug(f'lib_src is {text}')
        return text

    def set_lib_src(self, text):
        self.logger.debug(f'Set lib_src to {text}')
        self.ui.lib_src.setCurrentText(str(text))

    def get_user_val(self):
        val = self.ui.user_val.text()
        self.logger.debug(f'user_val is {val}')
        return val

    def set_user_val(self, val):
        self.logger.debug(f'Set user_val to {val}')
        self.ui.user_val.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_user_val()).strip() == '':
            errors.append('User specified value must not be empty.')
        return errors
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Revision Control System parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:rcs'
        for k, v in RCS_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Revision Control System Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='rcs.json', help='target config file.')
    ...
    win = MainWindow()
    op = RcsOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
