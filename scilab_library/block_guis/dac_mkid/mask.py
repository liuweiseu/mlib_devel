...
import os
from dac_mkid_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in dac_mkid_ui.py.
# The keys must match scilab_blocks/casper_xps/dac_mkid.json, since the
...
class DacMkidOperations(object):
    def __init__(self, winobj, template_config='dac_mkid.json', target_config='dac_mkid.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('dac_mkid.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('dac_mkid.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('DAC MKID Interface Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in DAC_MKID_CONFIG:
                setfunc = DAC_MKID_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid DAC MKID Interface parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:dac_mkid'
        for k, v in DAC_MKID_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for DAC MKID Interface Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='dac_mkid.json', help='target config file.')
    ...
    win = MainWindow()
    op = DacMkidOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
