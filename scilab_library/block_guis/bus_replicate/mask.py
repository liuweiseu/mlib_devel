...
import os
from bus_replicate_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_replicate_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_replicate.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
BUS_REPLICATE_CONFIG = {
    'n_bits':
    {
        'get': 'get_n_bits',
        'set': 'set_n_bits'
...
class BusReplicateOperations(object):
    def __init__(self, winobj, template_config='bus_replicate.json', target_config='bus_replicate.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_replicate.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_replicate.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Replicate Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_replication(self):
        val = self.ui.replication.text()
        self.logger.debug(f'replication is {val}')
        return val

    def set_replication(self, val):
        self.logger.debug(f'Set replication to {val}')
        self.ui.replication.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_n_bits()).strip() == '':
            errors.append('Bit Width (added for Xcos; not a casper_library mask param) must not be empty.')
        if str(self.get_replication()).strip() == '':
            errors.append('replication factor must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_REPLICATE_CONFIG:
                setfunc = BUS_REPLICATE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Replicate parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_replicate'
        for k, v in BUS_REPLICATE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Replicate Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_replicate.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusReplicateOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
