...
import os
from negedge_delay_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in negedge_delay_ui.py.
...
class NegedgeDelayOperations(object):
    def __init__(self, winobj, template_config='negedge_delay.json', target_config='negedge_delay.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('negedge_delay.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('negedge_delay.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Negedge Delay (Pulse Extend) Mask started.')
        self.logger.info('*************************************************')
    ...
    def set_pulse_len(self, val):
        self.logger.debug(f'Set pulse_len to {val}')
        self.ui.pulse_len.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in NEGEDGE_DELAY_CONFIG:
                setfunc = NEGEDGE_DELAY_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Negedge Delay (Pulse Extend) parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:negedge_delay'
        for k, v in NEGEDGE_DELAY_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Negedge Delay (Pulse Extend) Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='negedge_delay.json', help='target config file.')
    ...
    win = MainWindow()
    op = NegedgeDelayOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
