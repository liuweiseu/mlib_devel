...
import os
from complex_conj_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in complex_conj_ui.py.
# The keys must match scilab_blocks/casper_dsp/complex_conj.json, since the
...
class ComplexConjOperations(object):
    def __init__(self, winobj, template_config='complex_conj.json', target_config='complex_conj.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('complex_conj.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('complex_conj.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Complex Conjugate Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in COMPLEX_CONJ_CONFIG:
                setfunc = COMPLEX_CONJ_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Complex Conjugate parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:complex_conj'
        for k, v in COMPLEX_CONJ_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Complex Conjugate Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='complex_conj.json', help='target config file.')
    ...
    win = MainWindow()
    op = ComplexConjOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
