...
import os
from complex_convert_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in complex_convert_ui.py.
# The keys must match scilab_blocks/casper_dsp/complex_convert.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
COMPLEX_CONVERT_CONFIG = {
    'n_bits_in':
    {
        'get': 'get_n_bits_in',
        'set': 'set_n_bits_in'
...
class ComplexConvertOperations(object):
    def __init__(self, winobj, template_config='complex_convert.json', target_config='complex_convert.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('complex_convert.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('complex_convert.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Complex Convert Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in COMPLEX_CONVERT_CONFIG:
                setfunc = COMPLEX_CONVERT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Complex Convert parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:complex_convert'
        for k, v in COMPLEX_CONVERT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Complex Convert Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='complex_convert.json', help='target config file.')
    ...
    win = MainWindow()
    op = ComplexConvertOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
