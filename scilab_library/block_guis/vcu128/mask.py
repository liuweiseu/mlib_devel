...
import os
from vcu128_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in vcu128_ui.py.
# The keys must match scilab_blocks/casper_xps/vcu128.json, since the
...
class Vcu128Operations(object):
    def __init__(self, winobj, template_config='vcu128.json', target_config='vcu128.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('vcu128.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('vcu128.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('VCU128 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in VCU128_CONFIG:
                setfunc = VCU128_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid VCU128 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:vcu128'
        for k, v in VCU128_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for VCU128 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='vcu128.json', help='target config file.')
    ...
    win = MainWindow()
    op = Vcu128Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
