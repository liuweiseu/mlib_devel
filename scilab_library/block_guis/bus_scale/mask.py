...
import os
from bus_scale_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_scale_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_scale.json, since the
...
class BusScaleOperations(object):
    def __init__(self, winobj, template_config='bus_scale.json', target_config='bus_scale.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_scale.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_scale.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Scale Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_scale_factor(self):
        val = self.ui.scale_factor.text()
        self.logger.debug(f'scale_factor is {val}')
        return val

    def set_scale_factor(self, val):
        self.logger.debug(f'Set scale_factor to {val}')
        self.ui.scale_factor.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_type_in()).strip() == '':
            errors.append('input types must not be empty.')
        if str(self.get_scale_factor()).strip() == '':
            errors.append('scale factor (2^?) must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_SCALE_CONFIG:
                setfunc = BUS_SCALE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Scale parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_scale'
        for k, v in BUS_SCALE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Scale Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_scale.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusScaleOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
