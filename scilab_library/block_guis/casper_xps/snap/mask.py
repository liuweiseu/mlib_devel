...
import os
from snap_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in snap_ui.py.
# The keys must match scilab_blocks/casper_xps/snap.json, since the
...
class SnapOperations(object):
    def __init__(self, winobj, template_config='snap.json', target_config='snap.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('snap.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('snap.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('SNAP Mask started.')
        self.logger.info('*************************************************')
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid SNAP parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:snap'
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for SNAP Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='snap.json', help='target config file.')
    ...
    win = MainWindow()
    op = SnapOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
