...
import os
from of_detect_bus_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in of_detect_bus_ui.py.
...
class OfDetectBusOperations(object):
    def __init__(self, winobj, template_config='of_detect_bus.json', target_config='of_detect_bus.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('of_detect_bus.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('of_detect_bus.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('OF Detect Bus Mask started.')
        self.logger.info('*************************************************')
    ...
    def set_nStreams(self, val):
        self.logger.debug(f'Set nStreams to {val}')
        self.ui.nStreams.setText(str(val))
    ...
    def set_bit_width(self, val):
        ...
        self.ui.bit_width.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_nStreams()).strip() == '':
            errors.append('Number of Streams must not be empty.')
        if str(self.get_bit_width()).strip() == '':
            errors.append('Bit Width must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in OF_DETECT_BUS_CONFIG:
                setfunc = OF_DETECT_BUS_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid OF Detect Bus parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:of_detect_bus'
        for k, v in OF_DETECT_BUS_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for OF Detect Bus Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='of_detect_bus.json', help='target config file.')
    ...
    win = MainWindow()
    op = OfDetectBusOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
