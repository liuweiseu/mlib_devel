...
import os
from adm_pcie_9h7_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adm_pcie_9h7_ui.py.
# The keys must match scilab_blocks/casper_xps/adm_pcie_9h7.json, since the
...
class AdmPcie9h7Operations(object):
    def __init__(self, winobj, template_config='adm_pcie_9h7.json', target_config='adm_pcie_9h7.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adm_pcie_9h7.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adm_pcie_9h7.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('ADM_PCIE_9H7 Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_use_pr(self):
        status = self.ui.use_pr.isChecked()
        self.logger.debug(f'use_pr status is {status}')
        return self._convert_status(status)

    def set_use_pr(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set use_pr status to {status}')
        self.ui.use_pr.setChecked(status)

    def get_pr_template(self):
        val = self.ui.pr_template.text()
        self.logger.debug(f'pr_template is {val}')
        return val

    def set_pr_template(self, val):
        self.logger.debug(f'Set pr_template to {val}')
        self.ui.pr_template.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_pr_template()).strip() == '':
            errors.append('pr_template must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADM_PCIE_9H7_CONFIG:
                setfunc = ADM_PCIE_9H7_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid ADM_PCIE_9H7 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adm_pcie_9h7'
        for k, v in ADM_PCIE_9H7_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for ADM_PCIE_9H7 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adm_pcie_9h7.json', help='target config file.')
    ...
    win = MainWindow()
    op = AdmPcie9h7Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
