...
import os
from skarab_adc4x3g_14_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in skarab_adc4x3g_14_ui.py.
# The keys must match scilab_blocks/casper_xps/skarab_adc4x3g_14.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
SKARAB_ADC4X3G_14_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class SkarabAdc4x3g14Operations(object):
    def __init__(self, winobj, template_config='skarab_adc4x3g_14.json', target_config='skarab_adc4x3g_14.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('skarab_adc4x3g_14.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('skarab_adc4x3g_14.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('SKARAB ADC4x3G-14 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SKARAB_ADC4X3G_14_CONFIG:
                setfunc = SKARAB_ADC4X3G_14_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid SKARAB ADC4x3G-14 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:skarab_adc4x3g_14'
        for k, v in SKARAB_ADC4X3G_14_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for SKARAB ADC4x3G-14 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='skarab_adc4x3g_14.json', help='target config file.')
    ...
    win = MainWindow()
    op = SkarabAdc4x3g14Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
