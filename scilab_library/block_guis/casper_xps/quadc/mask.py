...
import os
from quadc_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in quadc_ui.py.
# The keys must match scilab_blocks/casper_xps/quadc.json, since the
...
class QuadcOperations(object):
    def __init__(self, winobj, template_config='quadc.json', target_config='quadc.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('quadc.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('quadc.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('quadc Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in QUADC_CONFIG:
                setfunc = QUADC_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid quadc parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:quadc'
        for k, v in QUADC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for quadc Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='quadc.json', help='target config file.')
    ...
    win = MainWindow()
    op = QuadcOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
