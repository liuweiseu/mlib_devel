...
import os
from mx175_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in mx175_ui.py.
# The keys must match scilab_blocks/casper_xps/mx175.json, since the
...
class Mx175Operations(object):
    def __init__(self, winobj, template_config='mx175.json', target_config='mx175.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('mx175.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('mx175.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('MX175 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in MX175_CONFIG:
                setfunc = MX175_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid MX175 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:mx175'
        for k, v in MX175_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for MX175 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='mx175.json', help='target config file.')
    ...
    win = MainWindow()
    op = Mx175Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
