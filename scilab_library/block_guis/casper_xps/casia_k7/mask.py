...
import os
from casia_k7_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in casia_k7_ui.py.
# The keys must match scilab_blocks/casper_xps/casia_k7.json, since the
...
class CasiaK7Operations(object):
    def __init__(self, winobj, template_config='casia_k7.json', target_config='casia_k7.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('casia_k7.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('casia_k7.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('CASIA_K7 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in CASIA_K7_CONFIG:
                setfunc = CASIA_K7_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid CASIA_K7 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:casia_k7'
        for k, v in CASIA_K7_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for CASIA_K7 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='casia_k7.json', help='target config file.')
    ...
    win = MainWindow()
    op = CasiaK7Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
