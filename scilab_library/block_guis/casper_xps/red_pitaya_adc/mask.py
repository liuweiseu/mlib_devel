...
import os
from red_pitaya_adc_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in red_pitaya_adc_ui.py.
# The keys must match scilab_blocks/casper_xps/red_pitaya_adc.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
RED_PITAYA_ADC_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class RedPitayaAdcOperations(object):
    def __init__(self, winobj, template_config='red_pitaya_adc.json', target_config='red_pitaya_adc.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('red_pitaya_adc.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('red_pitaya_adc.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Red Pitaya ADC Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_bits(self):
        text = self.ui.bits.currentText()
        self.logger.debug(f'bits is {text}')
        return text

    def set_bits(self, text):
        self.logger.debug(f'Set bits to {text}')
        self.ui.bits.setCurrentText(str(text))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in RED_PITAYA_ADC_CONFIG:
                setfunc = RED_PITAYA_ADC_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Red Pitaya ADC parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:red_pitaya_adc'
        for k, v in RED_PITAYA_ADC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Red Pitaya ADC Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='red_pitaya_adc.json', help='target config file.')
    ...
    win = MainWindow()
    op = RedPitayaAdcOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
