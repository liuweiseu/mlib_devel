...
import os
from adc_5g_e2v_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc_5g_e2v_ui.py.
# The keys must match scilab_blocks/casper_xps/adc_5g_e2v.json, since the
...
class Adc5gE2vOperations(object):
    def __init__(self, winobj, template_config='adc_5g_e2v.json', target_config='adc_5g_e2v.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc_5g_e2v.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc_5g_e2v.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc_5g_e2v Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_adc_mode(self):
        text = self.ui.adc_mode.currentText()
        self.logger.debug(f'adc_mode is {text}')
        return text

    def set_adc_mode(self, text):
        self.logger.debug(f'Set adc_mode to {text}')
        self.ui.adc_mode.setCurrentText(str(text))

    def get_output_mode(self):
        text = self.ui.output_mode.currentText()
        self.logger.debug(f'output_mode is {text}')
        return text

    def set_output_mode(self, text):
        self.logger.debug(f'Set output_mode to {text}')
        self.ui.output_mode.setCurrentText(str(text))

    def get_chips_num(self):
        text = self.ui.chips_num.currentText()
        self.logger.debug(f'chips_num is {text}')
        return text

    def set_chips_num(self, text):
        self.logger.debug(f'Set chips_num to {text}')
        self.ui.chips_num.setCurrentText(str(text))

    def get_adc_loc(self):
        text = self.ui.adc_loc.currentText()
        self.logger.debug(f'adc_loc is {text}')
        return text

    def set_adc_loc(self, text):
        self.logger.debug(f'Set adc_loc to {text}')
        self.ui.adc_loc.setCurrentText(str(text))

    def get_f_sample(self):
        val = self.ui.f_sample.text()
        self.logger.debug(f'f_sample is {val}')
        return val

    def set_f_sample(self, val):
        self.logger.debug(f'Set f_sample to {val}')
        self.ui.f_sample.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_f_sample()).strip() == '':
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC_5G_E2V_CONFIG:
                setfunc = ADC_5G_E2V_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc_5g_e2v parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc_5g_e2v'
        for k, v in ADC_5G_E2V_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc_5g_e2v Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc_5g_e2v.json', help='target config file.')
    ...
    win = MainWindow()
    op = Adc5gE2vOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
