...
import os
from dcp_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in dcp_ui.py.
...
class DcpOperations(object):
    def __init__(self, winobj, template_config='dcp.json', target_config='dcp.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('dcp.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('dcp.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('DCP Mask started.')
        self.logger.info('*************************************************')
    ...
    def set_dcp_file(self, val):
        self.logger.debug(f'Set dcp_file to {val}')
        self.ui.dcp_file.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_dcp_file()).strip() == '':
            errors.append('Name of the DCP file must not be empty.')
        return errors
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid DCP parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:dcp'
        for k, v in DCP_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for DCP Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='dcp.json', help='target config file.')
    ...
    win = MainWindow()
    op = DcpOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
