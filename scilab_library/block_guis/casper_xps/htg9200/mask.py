...
import os
from htg9200_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in htg9200_ui.py.
# The keys must match scilab_blocks/casper_xps/htg9200.json, since the
...
class Htg9200Operations(object):
    def __init__(self, winobj, template_config='htg9200.json', target_config='htg9200.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('htg9200.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('htg9200.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('HTG9200 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in HTG9200_CONFIG:
                setfunc = HTG9200_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid HTG9200 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:htg9200'
        for k, v in HTG9200_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for HTG9200 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='htg9200.json', help='target config file.')
    ...
    win = MainWindow()
    op = Htg9200Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
