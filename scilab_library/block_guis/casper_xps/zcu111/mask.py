...
import os
from zcu111_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in zcu111_ui.py.
# The keys must match scilab_blocks/casper_xps/zcu111.json, since the
...
class Zcu111Operations(object):
    def __init__(self, winobj, template_config='zcu111.json', target_config='zcu111.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('zcu111.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('zcu111.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('ZCU111 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ZCU111_CONFIG:
                setfunc = ZCU111_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid ZCU111 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:zcu111'
        for k, v in ZCU111_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for ZCU111 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='zcu111.json', help='target config file.')
    ...
    win = MainWindow()
    op = Zcu111Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
