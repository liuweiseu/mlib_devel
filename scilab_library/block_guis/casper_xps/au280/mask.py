...
import os
from au280_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in au280_ui.py.
# The keys must match scilab_blocks/casper_xps/au280.json, since the
...
class Au280Operations(object):
    def __init__(self, winobj, template_config='au280.json', target_config='au280.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('au280.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('au280.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('AU280 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in AU280_CONFIG:
                setfunc = AU280_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid AU280 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:au280'
        for k, v in AU280_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for AU280 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='au280.json', help='target config file.')
    ...
    win = MainWindow()
    op = Au280Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
