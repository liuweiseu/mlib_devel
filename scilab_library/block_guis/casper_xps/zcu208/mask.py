...
import os
from zcu208_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in zcu208_ui.py.
# The keys must match scilab_blocks/casper_xps/zcu208.json, since the
...
class Zcu208Operations(object):
    def __init__(self, winobj, template_config='zcu208.json', target_config='zcu208.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('zcu208.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('zcu208.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('ZCU208 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ZCU208_CONFIG:
                setfunc = ZCU208_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid ZCU208 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:zcu208'
        for k, v in ZCU208_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for ZCU208 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='zcu208.json', help='target config file.')
    ...
    win = MainWindow()
    op = Zcu208Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
