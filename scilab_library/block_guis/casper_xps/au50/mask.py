...
import os
from au50_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in au50_ui.py.
# The keys must match scilab_blocks/casper_xps/au50.json, since the
...
class Au50Operations(object):
    def __init__(self, winobj, template_config='au50.json', target_config='au50.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('au50.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('au50.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('AU50 Mask started.')
        self.logger.info('*************************************************')
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid AU50 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:au50'
        for k, v in AU50_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for AU50 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='au50.json', help='target config file.')
    ...
    win = MainWindow()
    op = Au50Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
