...
import os
from adc_conv_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc_conv_ui.py.
...
class AdcConvOperations(object):
    def __init__(self, winobj, template_config='adc_conv.json', target_config='adc_conv.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc_conv.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc_conv.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Conv Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC_CONV_CONFIG:
                setfunc = ADC_CONV_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Conv parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc_conv'
        for k, v in ADC_CONV_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Conv Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc_conv.json', help='target config file.')
    ...
    win = MainWindow()
    op = AdcConvOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
