...
import os
from cconv_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in cconv_ui.py.
# The keys must match scilab_blocks/casper_dsp/cconv.json, since the
...
class CconvOperations(object):
    def __init__(self, winobj, template_config='cconv.json', target_config='cconv.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('cconv.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('cconv.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Convert Unsigned to Twos Complement Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in CASPER_CONV_CONFIG:
                setfunc = CASPER_CONV_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Convert Unsigned to Twos Complement parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:cconv'
        for k, v in CASPER_CONV_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Convert Unsigned to Twos Complement Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='cconv.json', help='target config file.')
    ...
    win = MainWindow()
    op = CconvOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
