...
import os
from power_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in power_ui.py.
# The keys must match scilab_blocks/casper_dsp/power.json, since the
...
class PowerOperations(object):
    def __init__(self, winobj, template_config='power.json', target_config='power.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('power.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('power.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Power Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in POWER_CONFIG:
                setfunc = POWER_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Power parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:power'
        for k, v in POWER_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Power Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='power.json', help='target config file.')
    ...
    win = MainWindow()
    op = PowerOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
