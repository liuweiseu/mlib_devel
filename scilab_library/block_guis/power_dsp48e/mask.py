...
import os
from power_dsp48e_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in power_dsp48e_ui.py.
# The keys must match scilab_blocks/casper_dsp/power_dsp48e.json, since the
...
class PowerDsp48eOperations(object):
    def __init__(self, winobj, template_config='power_dsp48e.json', target_config='power_dsp48e.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('power_dsp48e.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('power_dsp48e.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Power (DSP48E) Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in POWER_DSP48E_CONFIG:
                setfunc = POWER_DSP48E_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Power (DSP48E) parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:power_dsp48e'
        for k, v in POWER_DSP48E_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Power (DSP48E) Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='power_dsp48e.json', help='target config file.')
    ...
    win = MainWindow()
    op = PowerDsp48eOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
