...
import os
from pulse_ext_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in pulse_ext_ui.py.
# The keys must match scilab_blocks/casper_dsp/pulse_ext.json, since the
...
class PulseExtOperations(object):
    def __init__(self, winobj, template_config='pulse_ext.json', target_config='pulse_ext.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('pulse_ext.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('pulse_ext.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Pulse Extend Mask started.')
        self.logger.info('*************************************************')
    ...
    # *******************************************************************
    def get_length(self):
        val = self.ui.length.value()
        self.logger.debug(f'length is {val}')
        return str(val)

    def set_length(self, val):
        self.logger.debug(f'Set length to {val}')
        self.ui.length.setValue(int(val))
    ...
    def validate_config(self):
        ...
        casper_library's pulse_ext mask has no explicit range check on
        pulse_len beyond it being usable as a bit-width count
        (bits = ceil(log2(pulse_len+1))) -- the spinbox's spin_range=[1, 2^31-1]
        already keeps Length a positive integer, so there's nothing further
        to validate here.
        """
        errors = []
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in PULSE_EXT_CONFIG:
                setfunc = PULSE_EXT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Pulse Extend parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:pulse_ext'
        for k, v in PULSE_EXT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Pulse Extend Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='pulse_ext.json', help='target config file.')
    ...
    win = MainWindow()
    op = PulseExtOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
