...
import os
from of1_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in of1_ui.py.
# The keys must match scilab_blocks/casper_dsp/of1.json, since the
...
class Of1Operations(object):
    def __init__(self, winobj, template_config='of1.json', target_config='of1.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('of1.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('of1.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('1-Cycle Overflow Detector Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_bit_num(self):
        val = self.ui.bit_num.value()
        self.logger.debug(f'bit_num is {val}')
        return str(val)

    def set_bit_num(self, val):
        self.logger.debug(f'Set bit_num to {val}')
        self.ui.bit_num.setValue(int(val))

    def get_n_adc_bits(self):
        val = self.ui.n_adc_bits.value()
        self.logger.debug(f'n_adc_bits is {val}')
        return str(val)

    def set_n_adc_bits(self, val):
        self.logger.debug(f'Set n_adc_bits to {val}')
        self.ui.n_adc_bits.setValue(int(val))
    ...
    def validate_config(self):
        ...
        The overflow detection bit index must fall within the ADC's own bit
        width.
        """
        errors = []
        if int(self.get_bit_num()) >= int(self.get_n_adc_bits()):
            errors.append('Bit index of overflow detection bus must be less '
                          'than Number of bits in ADC.')
        return errors
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid 1-Cycle Overflow Detector parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:of1'
        for k, v in OF1_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for 1-Cycle Overflow Detector Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='of1.json', help='target config file.')
    ...
    win = MainWindow()
    op = Of1Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
