...
import os
from casia_k7_21cma_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in casia_k7_21cma_ui.py.
# The keys must match scilab_blocks/casper_xps/casia_k7_21cma.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
CASIA_K7_21CMA_CONFIG = {
    'hw_sys':
    {
        'get': 'get_hw_sys',
        'set': 'set_hw_sys'
...
class CasiaK721cmaOperations(object):
    def __init__(self, winobj, template_config='casia_k7_21cma.json', target_config='casia_k7_21cma.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('casia_k7_21cma.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('casia_k7_21cma.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('CASIA_K7_21CMA Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in CASIA_K7_21CMA_CONFIG:
                setfunc = CASIA_K7_21CMA_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid CASIA_K7_21CMA parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:casia_k7_21cma'
        for k, v in CASIA_K7_21CMA_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for CASIA_K7_21CMA Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='casia_k7_21cma.json', help='target config file.')
    ...
    win = MainWindow()
    op = CasiaK721cmaOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
