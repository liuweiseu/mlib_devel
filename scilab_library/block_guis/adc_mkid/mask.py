...
import os
from adc_mkid_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc_mkid_ui.py.
# The keys must match scilab_blocks/casper_xps/adc_mkid.json, since the
...
class AdcMkidOperations(object):
    def __init__(self, winobj, template_config='adc_mkid.json', target_config='adc_mkid.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc_mkid.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc_mkid.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc_mkid Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC_MKID_CONFIG:
                setfunc = ADC_MKID_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc_mkid parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc_mkid'
        for k, v in ADC_MKID_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc_mkid Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc_mkid.json', help='target config file.')
    ...
    win = MainWindow()
    op = AdcMkidOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
