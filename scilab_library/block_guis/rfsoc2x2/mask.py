...
import os
from rfsoc2x2_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in rfsoc2x2_ui.py.
# The keys must match scilab_blocks/casper_xps/rfsoc2x2.json, since the
...
class Rfsoc2x2Operations(object):
    def __init__(self, winobj, template_config='rfsoc2x2.json', target_config='rfsoc2x2.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('rfsoc2x2.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('rfsoc2x2.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('RFSoC2x2 Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_pl_clk_rate(self):
        val = self.ui.pl_clk_rate.text()
        self.logger.debug(f'pl_clk_rate is {val}')
        return val

    def set_pl_clk_rate(self, val):
        self.logger.debug(f'Set pl_clk_rate to {val}')
        self.ui.pl_clk_rate.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_pl_clk_rate()).strip() == '':
            errors.append('RFPLL PL Clock Rate (MHz) must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in RFSOC2X2_CONFIG:
                setfunc = RFSOC2X2_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid RFSoC2x2 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:rfsoc2x2'
        for k, v in RFSOC2X2_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for RFSoC2x2 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='rfsoc2x2.json', help='target config file.')
    ...
    win = MainWindow()
    op = Rfsoc2x2Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
