...
import os
from bus_convert_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_convert_ui.py.
...
class BusConvertOperations(object):
    def __init__(self, winobj, template_config='bus_convert.json', target_config='bus_convert.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_convert.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_convert.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Convert Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_of(self):
        status = self.ui.of.isChecked()
        self.logger.debug(f'of status is {status}')
        return self._convert_status(status)

    def set_of(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set of status to {status}')
        self.ui.of.setChecked(status)
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_CONVERT_CONFIG:
                setfunc = BUS_CONVERT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Convert parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_convert'
        for k, v in BUS_CONVERT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Convert Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_convert.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusConvertOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
