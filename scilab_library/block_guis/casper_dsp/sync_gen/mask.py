...
import os
from sync_gen_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in sync_gen_ui.py.
# The keys must match scilab_blocks/casper_dsp/sync_gen.json, since the
...
SYNC_GEN_CONFIG = {
    ...
    'fft_simult_inputs':
    {
        'get': 'get_fft_simult_inputs',
        'set': 'set_fft_simult_inputs'
    },
    'pfb_fir_taps':
    {
        'get': 'get_pfb_fir_taps',
        'set': 'set_pfb_fir_taps'
    },
    'reorder_vec':
    {
        'get': 'get_reorder_vec',
...
class SyncGenOperations(object):
    def __init__(self, winobj, template_config='sync_gen.json', target_config='sync_gen.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('sync_gen.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('sync_gen.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Sync Generator Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_sim_acc_len(self):
        val = self.ui.sim_acc_len.value()
        self.logger.debug(f'sim_acc_len is {val}')
        return str(val)

    def set_sim_acc_len(self, val):
        self.logger.debug(f'Set sim_acc_len to {val}')
        self.ui.sim_acc_len.setValue(int(val))

    def get_fft_size(self):
        val = self.ui.fft_size.value()
        self.logger.debug(f'fft_size is {val}')
        return str(val)

    def set_fft_size(self, val):
        self.logger.debug(f'Set fft_size to {val}')
        self.ui.fft_size.setValue(int(val))

    def get_fft_simult_inputs(self):
        val = self.ui.fft_simult_inputs.value()
        self.logger.debug(f'fft_simult_inputs is {val}')
        return str(val)

    def set_fft_simult_inputs(self, val):
        self.logger.debug(f'Set fft_simult_inputs to {val}')
        self.ui.fft_simult_inputs.setValue(int(val))

    def get_pfb_fir_taps(self):
        val = self.ui.pfb_fir_taps.value()
        self.logger.debug(f'pfb_fir_taps is {val}')
        return str(val)

    def set_pfb_fir_taps(self, val):
        self.logger.debug(f'Set pfb_fir_taps to {val}')
        self.ui.pfb_fir_taps.setValue(int(val))

    def get_reorder_vec(self):
        val = self.ui.reorder_vec.text()
        self.logger.debug(f'reorder_vec is {val}')
        return val

    def set_reorder_vec(self, val):
        self.logger.debug(f'Set reorder_vec to {val}')
        self.ui.reorder_vec.setText(str(val))

    def get_scale(self):
        val = self.ui.scale.value()
        self.logger.debug(f'scale is {val}')
        return str(val)

    def set_scale(self, val):
        self.logger.debug(f'Set scale to {val}')
        self.ui.scale.setValue(int(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_reorder_vec()).strip() == '':
            errors.append('Reorder Orders (e.g. [2 2 2]) must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SYNC_GEN_CONFIG:
                setfunc = SYNC_GEN_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Sync Generator parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:sync_gen'
        for k, v in SYNC_GEN_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Sync Generator Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='sync_gen.json', help='target config file.')
    ...
    win = MainWindow()
    op = SyncGenOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
