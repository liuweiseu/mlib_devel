...
import os
from armed_trigger_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in armed_trigger_ui.py.
# The keys must match scilab_blocks/casper_dsp/armed_trigger.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
ARMED_TRIGGER_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class ArmedTriggerOperations(object):
    def __init__(self, winobj, template_config='armed_trigger.json', target_config='armed_trigger.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('armed_trigger.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('armed_trigger.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Armed Trigger Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        armed_trigger has no configurable parameters in casper_library (no
        MaskParameter/Initialization in its mask), so there is nothing to
        validate here.
        """
        errors = []
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ARMED_TRIGGER_CONFIG:
                setfunc = ARMED_TRIGGER_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Armed Trigger parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:armed_trigger'
        for k, v in ARMED_TRIGGER_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Armed Trigger Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='armed_trigger.json', help='target config file.')
    ...
    win = MainWindow()
    op = ArmedTriggerOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
