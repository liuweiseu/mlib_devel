...
import os
from freeze_cntr_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in freeze_cntr_ui.py.
# The keys must match scilab_blocks/casper_dsp/freeze_cntr.json, since the
...
FREEZE_CNTR_CONFIG = {
    ...
    'counter_bits':
    {
        'get': 'get_counter_bits',
        'set': 'set_counter_bits'
    },
}
...
class FreezeCntrOperations(object):
    def __init__(self, winobj, template_config='freeze_cntr.json', target_config='freeze_cntr.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('freeze_cntr.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('freeze_cntr.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Freeze Counter Mask started.')
        self.logger.info('*************************************************')
    ...
    # Low-level APIs
    # *******************************************************************
    def get_counter_bits(self):
        val = self.ui.counter_bits.value()
        self.logger.debug(f'counter_bits is {val}')
        return str(val)

    def set_counter_bits(self, val):
        self.logger.debug(f'Set counter_bits to {val}')
        self.ui.counter_bits.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in FREEZE_CNTR_CONFIG:
                setfunc = FREEZE_CNTR_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Freeze Counter parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:freeze_cntr'
        for k, v in FREEZE_CNTR_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Freeze Counter Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='freeze_cntr.json', help='target config file.')
    ...
    win = MainWindow()
    op = FreezeCntrOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
