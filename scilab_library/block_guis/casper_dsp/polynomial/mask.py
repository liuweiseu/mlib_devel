...
import os
from polynomial_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in polynomial_ui.py.
# The keys must match scilab_blocks/casper_dsp/polynomial.json, since the
...
class PolynomialOperations(object):
    def __init__(self, winobj, template_config='polynomial.json', target_config='polynomial.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('polynomial.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('polynomial.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Polynomial Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_n_polys(self):
        val = self.ui.n_polys.value()
        self.logger.debug(f'n_polys is {val}')
        return str(val)

    def set_n_polys(self, val):
        self.logger.debug(f'Set n_polys to {val}')
        self.ui.n_polys.setValue(int(val))

    def get_degree(self):
        val = self.ui.degree.value()
        self.logger.debug(f'degree is {val}')
        return str(val)

    def set_degree(self, val):
        self.logger.debug(f'Set degree to {val}')
        self.ui.degree.setValue(int(val))
    ...
    def get_bits_out(self):
        val = self.ui.bits_out.value()
        self.logger.debug(f'bits_out is {val}')
        return str(val)

    def set_bits_out(self, val):
        self.logger.debug(f'Set bits_out to {val}')
        self.ui.bits_out.setValue(int(val))
    ...
    def get_bits_in(self):
        val = self.ui.bits_in.value()
        self.logger.debug(f'bits_in is {val}')
        return str(val)

    def set_bits_in(self, val):
        self.logger.debug(f'Set bits_in to {val}')
        self.ui.bits_in.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in POLYNOMIAL_CONFIG:
                setfunc = POLYNOMIAL_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Polynomial parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:polynomial'
        for k, v in POLYNOMIAL_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Polynomial Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='polynomial.json', help='target config file.')
    ...
    win = MainWindow()
    op = PolynomialOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
