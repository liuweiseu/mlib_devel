...
import os
from sample_and_hold_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in sample_and_hold_ui.py.
...
class SampleAndHoldOperations(object):
    def __init__(self, winobj, template_config='sample_and_hold.json', target_config='sample_and_hold.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('sample_and_hold.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('sample_and_hold.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Sample and Hold Mask started.')
        self.logger.info('*************************************************')
    ...
    def set_period(self, val):
        self.logger.debug(f'Set period to {val}')
        self.ui.period.setValue(int(val))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SAMPLE_AND_HOLD_CONFIG:
                setfunc = SAMPLE_AND_HOLD_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Sample and Hold parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:sample_and_hold'
        for k, v in SAMPLE_AND_HOLD_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Sample and Hold Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='sample_and_hold.json', help='target config file.')
    ...
    win = MainWindow()
    op = SampleAndHoldOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
