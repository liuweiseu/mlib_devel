...
import os
from bus_accumulator_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_accumulator_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_accumulator.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
BUS_ACCUMULATOR_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class BusAccumulatorOperations(object):
    def __init__(self, winobj, template_config='bus_accumulator.json', target_config='bus_accumulator.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_accumulator.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_accumulator.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Accumulator Mask started.')
        self.logger.info('*************************************************')
    ...
    # *******************************************************************
    def get_reset(self):
        status = self.ui.reset.isChecked()
        self.logger.debug(f'reset status is {status}')
        return self._convert_status(status)

    def set_reset(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set reset status to {status}')
        self.ui.reset.setChecked(status)

    def get_enable(self):
        status = self.ui.enable.isChecked()
        self.logger.debug(f'enable status is {status}')
        return self._convert_status(status)

    def set_enable(self, status):
        status = self._convert_status(status)
        self.logger.debug(f'Set enable status to {status}')
        self.ui.enable.setChecked(status)
    ...
    def get_type_in(self):
        val = self.ui.type_in.text()
        self.logger.debug(f'type_in is {val}')
        return val

    def set_type_in(self, val):
        self.logger.debug(f'Set type_in to {val}')
        self.ui.type_in.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_type_in()).strip() == '':
            errors.append('input types (Unsigned=0, Signed=1) must not be empty.')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_ACCUMULATOR_CONFIG:
                setfunc = BUS_ACCUMULATOR_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Accumulator parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_accumulator'
        for k, v in BUS_ACCUMULATOR_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Accumulator Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_accumulator.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusAccumulatorOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
