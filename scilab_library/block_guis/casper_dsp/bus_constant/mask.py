...
import os
from bus_constant_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bus_constant_ui.py.
# The keys must match scilab_blocks/casper_dsp/bus_constant.json, since the
...
class BusConstantOperations(object):
    def __init__(self, winobj, template_config='bus_constant.json', target_config='bus_constant.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bus_constant.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bus_constant.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bus Constant Mask started.')
        self.logger.info('*************************************************')
    ...
    # *******************************************************************
    def get_values(self):
        val = self.ui.values.text()
        self.logger.debug(f'values is {val}')
        return val

    def set_values(self, val):
        self.logger.debug(f'Set values to {val}')
        self.ui.values.setText(str(val))
    ...
    def get_types(self):
        val = self.ui.types.text()
        self.logger.debug(f'types is {val}')
        return val

    def set_types(self, val):
        self.logger.debug(f'Set types to {val}')
        self.ui.types.setText(str(val))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        """
        errors = []
        if str(self.get_values()).strip() == '':
            errors.append('values must not be empty.')
        if str(self.get_n_bits()).strip() == '':
            errors.append('bit widths must not be empty.')
        if str(self.get_bin_pts()).strip() == '':
            errors.append('binary points must not be empty.')
        if str(self.get_types()).strip() == '':
            errors.append('data type must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BUS_CONSTANT_CONFIG:
                setfunc = BUS_CONSTANT_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bus Constant parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bus_constant'
        for k, v in BUS_CONSTANT_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bus Constant Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bus_constant.json', help='target config file.')
    ...
    win = MainWindow()
    op = BusConstantOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
