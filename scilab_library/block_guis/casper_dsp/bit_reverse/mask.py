...
import os
from bit_reverse_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in bit_reverse_ui.py.
# The keys must match scilab_blocks/casper_dsp/bit_reverse.json, since the
...
class BitReverseOperations(object):
    def __init__(self, winobj, template_config='bit_reverse.json', target_config='bit_reverse.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('bit_reverse.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('bit_reverse.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bit Reverse Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in BIT_REVERSE_CONFIG:
                setfunc = BIT_REVERSE_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bit Reverse parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:bit_reverse'
        for k, v in BIT_REVERSE_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bit Reverse Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='bit_reverse.json', help='target config file.')
    ...
    win = MainWindow()
    op = BitReverseOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
