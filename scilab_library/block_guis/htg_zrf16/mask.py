...
import os
from htg_zrf16_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in htg_zrf16_ui.py.
# The keys must match scilab_blocks/casper_xps/htg_zrf16.json, since the
...
class HtgZrf16Operations(object):
    def __init__(self, winobj, template_config='htg_zrf16.json', target_config='htg_zrf16.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('htg_zrf16.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('htg_zrf16.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('HTG_ZRF16 Mask started.')
        self.logger.info('*************************************************')
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in HTG_ZRF16_CONFIG:
                setfunc = HTG_ZRF16_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid HTG_ZRF16 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:htg_zrf16'
        for k, v in HTG_ZRF16_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for HTG_ZRF16 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='htg_zrf16.json', help='target config file.')
    ...
    win = MainWindow()
    op = HtgZrf16Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
