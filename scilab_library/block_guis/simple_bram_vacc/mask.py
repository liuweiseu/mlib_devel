...
import os
from simple_bram_vacc_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in simple_bram_vacc_ui.py.
# The keys must match scilab_blocks/casper_dsp/simple_bram_vacc.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
SIMPLE_BRAM_VACC_CONFIG = {
    'vec_len':
    {
        'get': 'get_vec_len',
        'set': 'set_vec_len'
...
class SimpleBramVaccOperations(object):
    def __init__(self, winobj, template_config='simple_bram_vacc.json', target_config='simple_bram_vacc.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('simple_bram_vacc.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('simple_bram_vacc.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Simple BRAM Vector Accumulator Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        Rules ported from casper_library/simple_bram_vacc_init.m.
        """
        errors = []
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SIMPLE_BRAM_VACC_CONFIG:
                setfunc = SIMPLE_BRAM_VACC_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Simple BRAM Vector Accumulator parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'dsp:simple_bram_vacc'
        for k, v in SIMPLE_BRAM_VACC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Simple BRAM Vector Accumulator Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='simple_bram_vacc.json', help='target config file.')
    ...
    win = MainWindow()
    op = SimpleBramVaccOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
