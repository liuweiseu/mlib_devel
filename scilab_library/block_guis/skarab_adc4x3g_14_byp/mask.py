...
import os
from skarab_adc4x3g_14_byp_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in skarab_adc4x3g_14_byp_ui.py.
# The keys must match scilab_blocks/casper_xps/skarab_adc4x3g_14_byp.json, since the
# Scilab side (update_exprs) only reads back the keys listed in that template.
SKARAB_ADC4X3G_14_BYP_CONFIG = {
    'name':
    {
        'get': 'get_name',
        'set': 'set_name'
...
class SkarabAdc4x3g14BypOperations(object):
    def __init__(self, winobj, template_config='skarab_adc4x3g_14_byp.json', target_config='skarab_adc4x3g_14_byp.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('skarab_adc4x3g_14_byp.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('skarab_adc4x3g_14_byp.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('SKARAB ADC4x3G-14 (Bypass) Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_sync_ms(self):
        text = self.ui.sync_ms.currentText()
        self.logger.debug(f'sync_ms is {text}')
        return text

    def set_sync_ms(self, text):
        self.logger.debug(f'Set sync_ms to {text}')
        self.ui.sync_ms.setCurrentText(str(text))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SKARAB_ADC4X3G_14_BYP_CONFIG:
                setfunc = SKARAB_ADC4X3G_14_BYP_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid SKARAB ADC4x3G-14 (Bypass) parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:skarab_adc4x3g_14_byp'
        for k, v in SKARAB_ADC4X3G_14_BYP_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for SKARAB ADC4x3G-14 (Bypass) Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='skarab_adc4x3g_14_byp.json', help='target config file.')
    ...
    win = MainWindow()
    op = SkarabAdc4x3g14BypOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
