...
import os
from snap_adc_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in snap_adc_ui.py.
# The keys must match scilab_blocks/casper_xps/snap_adc.json, since the
...
SNAP_ADC_CONFIG = {
    ...
    'adc_resolution':
    {
        'get': 'get_adc_resolution',
        'set': 'set_adc_resolution'
    },
}
...
class SnapAdcOperations(object):
    def __init__(self, winobj, template_config='snap_adc.json', target_config='snap_adc.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('snap_adc.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('snap_adc.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('SNAP ADC Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_snap_inputs(self):
        text = self.ui.snap_inputs.currentText()
        self.logger.debug(f'snap_inputs is {text}')
        return text

    def set_snap_inputs(self, text):
        self.logger.debug(f'Set snap_inputs to {text}')
        self.ui.snap_inputs.setCurrentText(str(text))

    def get_adc_resolution(self):
        text = self.ui.adc_resolution.currentText()
        self.logger.debug(f'adc_resolution is {text}')
        return text

    def set_adc_resolution(self, text):
        self.logger.debug(f'Set adc_resolution to {text}')
        self.ui.adc_resolution.setCurrentText(str(text))
    ...
    def validate_config(self):
        ...
        *_maskcheck.m for this block (see the skill's extracted mask info).
        ...
        if str(self.get_sample_rate()).strip() == '':
            errors.append('Sample rate (MHz) must not be empty.')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in SNAP_ADC_CONFIG:
                setfunc = SNAP_ADC_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid SNAP ADC parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:snap_adc'
        for k, v in SNAP_ADC_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for SNAP ADC Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='snap_adc.json', help='target config file.')
    ...
    win = MainWindow()
    op = SnapAdcOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
