...
import os
from adc16x250_8_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in adc16x250_8_ui.py.
# The keys must match scilab_blocks/casper_xps/adc16x250_8.json, since the
...
class Adc16x2508Operations(object):
    def __init__(self, winobj, template_config='adc16x250_8.json', target_config='adc16x250_8.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('adc16x250_8.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('adc16x250_8.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('adc16x250_8 Mask started.')
        self.logger.info('*************************************************')
    ...
    def get_roach2_rev(self):
        text = self.ui.roach2_rev.currentText()
        self.logger.debug(f'roach2_rev is {text}')
        return text

    def set_roach2_rev(self, text):
        self.logger.debug(f'Set roach2_rev to {text}')
        self.ui.roach2_rev.setCurrentText(str(text))

    def get_zdok_rev(self):
        text = self.ui.zdok_rev.currentText()
        self.logger.debug(f'zdok_rev is {text}')
        return text

    def set_zdok_rev(self, text):
        self.logger.debug(f'Set zdok_rev to {text}')
        self.ui.zdok_rev.setCurrentText(str(text))
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in ADC16X250_8_CONFIG:
                setfunc = ADC16X250_8_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid adc16x250_8 parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:adc16x250_8'
        for k, v in ADC16X250_8_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for adc16x250_8 Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='adc16x250_8.json', help='target config file.')
    ...
    win = MainWindow()
    op = Adc16x2508Operations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
