...
import os
from gpio_bidir_ui import Ui_MainWindow
...
# synchronize them with the Qt widgets defined in gpio_bidir_ui.py.
# The keys must match scilab_blocks/casper_xps/gpio_bidir.json, since the
...
class GpioBidirOperations(object):
    def __init__(self, winobj, template_config='gpio_bidir.json', target_config='gpio_bidir.json', logdir='.', debug=False):
        if debug:
            self.logger = make_rich_logger('gpio_bidir.log', logging.DEBUG, mode='a', logdir=logdir)
        else:
            self.logger = make_rich_logger('gpio_bidir.log', logging.ERROR, mode='a', logdir=logdir)
        self.logger.info('*************************************************')
        self.logger.info('Bidirectional GPIO Mask started.')
        self.logger.info('*************************************************')
    ...
    def validate_config(self):
        ...
        Rule ported from xps_library/gpio_bidir_mask.m: bit_index's element
        count must equal bitwidth exactly.
        """
        errors = []
        ...
        if bit_index_len != int(self.get_bitwidth()):
            errors.append('GPIO bit index does not have the same number of elements as the '
                          'I/O bitwidth. When using bitwidths greater than one, specify a '
                          'vector of bit indices (e.g. "[0, 1, 2, 3]").')
        return errors
    ...
    def load_config(self):
        ...
        for k, v in parameters.items():
            if k in GPIO_BIDIR_CONFIG:
                setfunc = GPIO_BIDIR_CONFIG[k]['set']
    ...
    def collect_config(self):
        ...
        if errors:
            for e in errors:
                self.logger.error(e)
            QMessageBox.critical(self.winobj, 'Invalid Bidirectional GPIO parameters', chr(10).join(errors))
            return False
        self.logger.info('-------------------------------------------------')
        ...
        try:
            ...
            config['parameters']['tag'] = 'xps:gpio_bidir'
        for k, v in GPIO_BIDIR_CONFIG.items():
            getfunc = v['get']
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Usage for Bidirectional GPIO Mask.")
    parser.add_argument('--template', type=str, dest='template', default=None, help='template config file.')
    parser.add_argument('--target', type=str, dest='target', default='gpio_bidir.json', help='target config file.')
    ...
    win = MainWindow()
    op = GpioBidirOperations(win, opts.template, opts.target, opts.log, opts.debug)
    win.show()
    sys.exit(app.exec())
